...
- один активный запрос за раз (остальные ждут до таймаута);
...
1. `yaml-mcp-server` вызывает `POST /approve` и удерживает соединение.
2. `telegram-approver` отправляет сообщение в Telegram.
3. Пользователь выбирает решение или отправляет причину отказа.
4. `telegram-approver` возвращает JSON с `decision` и `reason`.

Если ожидание превысило таймаут — возвращается `decision=error` и сообщение о таймауте, кнопки удаляются, в Telegram добавляется соответствующая пометка.
...
- `TG_APPROVER_HTTP_ADDR` — адрес HTTP‑сервера (по умолчанию `:8080`)
...
Допустимые решения: `approve`, `deny`, `error`.
...
- При `Deny with message` бот ждёт текст или голос, затем передаёт причину в `reason`.
...
- Поддерживается **только один активный запрос**.
